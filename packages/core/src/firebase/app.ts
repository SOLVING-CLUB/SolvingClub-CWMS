...
import {
  getStorage, connectStorageEmulator, type FirebaseStorage,
...
export function initFirebase(config: Record<string, string>): FirebaseServices {
  ...
  return {
    app, auth: getAuth(app), db: getFirestore(app),
    functions: getFunctions(app), storage: getStorage(app),
  };
}
...
export function connectToEmulators(s: FirebaseServices): void {
  ...
  connectStorageEmulator(s.storage, "localhost", 9199);
}

...
function AppTasks({ application, tasks }: { application: Application; tasks: Task[] }) {
  ...
  if (error) return <p className="empty-state">Tasks could not be loaded. Sign out and back in, then retry.</p>;
  return (
  ...
  return (
    <div className="portal-shell">
      <aside className="portal-sidebar">
        ...
        <nav><a className="active" href="#overview"><LayoutDashboard /> Overview</a><a href="#documents"><FileText /> Documents</a><a href="#invoices"><ReceiptText /> Invoices</a></nav>
        <div className="portal-footer"><ThemeToggle /><Button variant="ghost" onPress={() => signOut(fb.auth)}><LogOut /> Sign out</Button></div>
      </aside>
      <section className="portal-main">
        <header className="workspace-toolbar"><div className="toolbar-context"><span>Client portal</span><i>/</i><strong>Overview</strong></div><div className="toolbar-actions"><NotificationBell subscribeNotifications={(onData, onError) => subscribeClientNotifications(db, clientId, onData, onError)} /></div></header>
        <main className="workspace-content content-stack" id="overview">
          ...
          <section className="portal-delivery-summary" aria-label="Delivery summary">
            <div><span>Projects</span><strong>{projects.length}</strong><small>Delivery streams</small></div>
            <div><span>Applications</span><strong>{applications.length}</strong><small>Active workspaces</small></div>
          ...
          <section><div className="section-heading"><div><h2>Your projects</h2><p>Current delivery work, applications, and task status.</p></div></div>
          ...
          <section id="documents" className="portal-section"><div className="section-heading"><div><h2>Documents</h2><p>Files and links shared with your account.</p></div></div><div className="portal-surface"><Documents ownerType="client" ownerId={clientId} clientId={clientId} canEdit={false} /></div></section>

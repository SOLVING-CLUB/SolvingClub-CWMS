      ...
      {tasks.map((task) => {
        ...
        return <article key={task.id} className={`application-task${openComments === task.id ? " expanded" : ""}${overdue ? " overdue" : ""}`}>
          <div className="application-task-row">
            ...
            <time className={overdue ? "overdue" : ""} dateTime={task.dueDate ? new Date(task.dueDate).toISOString() : undefined}><CalendarClock /><span>{dueLabel(task.dueDate)}</span><small>{task.assigneeUid ? memberNames.get(task.assigneeUid) ?? "Assigned" : "Unassigned"}</small></time>
